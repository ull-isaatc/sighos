...
//		sim.addListener(new SimulationStateListener());
		...
		for (int i = 1; i < nExperiments; i++) {
			...
			System.out.println("SIMULATION STORED\tID : " + dbStateProcessor.getSimulationId());
			Simulation sim = getSimulation(i);
			SimulationState state = dbStateProcessor.getState(dbStateProcessor.getSimulationId());
			System.out.println("SIMULATION LOADED\tID : " + dbStateProcessor.getSimulationId());
			sim.start(state);
			prevSim = sim;
			System.out.println("---------------------------------------------------------------");
		}
 ...
 *
 */
public class TestDBStateProcessor {
	/**
	 * @param args
	 */
