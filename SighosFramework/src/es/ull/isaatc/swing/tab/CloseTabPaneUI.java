...
public class CloseTabPaneUI extends BasicTabbedPaneUI {
	...
	public CloseTabPaneUI() {
		...
		try {
//            closeImgI = ImageIO.read(getClass().getResource("es/ull/cyc/swing/resources/delete_edit.gif"));
            closeImgI = ImageIO.read(getClass().getResource("../resources/delete_edit.gif"));
	 ...
	 * JTabbedPane textField. This is required because the tab rects are by
	 * default defined in the coordinate space of the textField where they are
		...
		public void layoutContainer(Container parent) {
			...
			if (selectedIndex < 0) {
				if (visibleComponent != null) {
					// The last tab was removed, so remove the textField
				...
				// In order to allow programs to use a single textField
				// as the display for multiple tabs, we will not change
				// the visible compnent if the currently selected tab
				// has a null textField. This is a bit dicey, as we don't
	 ...
	 * textField has a chance to complete the add) which checks the tab title of
	 * the new textField to see if it requires HTML rendering.
	 * 
