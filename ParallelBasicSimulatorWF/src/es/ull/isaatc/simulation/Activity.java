...
public abstract class Activity extends TimeStampedSimulationObject implements Prioritizable, Describable {
     ...
     */
    public int addWorkGroup(int priority, es.ull.isaatc.simulation.WorkGroup wg) {
     ...
     * @return The new workgroup's identifier.
     */
    public int addWorkGroup(int priority, es.ull.isaatc.simulation.WorkGroup wg, Condition cond) {
    	int wgId = workGroupTable.size();
     ...
     * @return The new workgroup's identifier.
     */
    public int addWorkGroup(es.ull.isaatc.simulation.WorkGroup wg) {    	
     ...
     * @return The new workgroup's identifier.
     */
    public int addWorkGroup(es.ull.isaatc.simulation.WorkGroup wg, Condition cond) {    	
	 ...
	 */
	public class ActivityWorkGroup extends es.ull.isaatc.simulation.WorkGroup implements Identifiable, Prioritizable, Describable, Comparable<ActivityWorkGroup> {
	     ...
	     */    
	    protected ActivityWorkGroup(int id, int priority, es.ull.isaatc.simulation.WorkGroup wg) {
	        super();
	        this.id = id;
	        this.priority = priority;
	     ...
	     * @param cond  Availability condition
	     */    
	    protected ActivityWorkGroup(int id, int priority, es.ull.isaatc.simulation.WorkGroup wg, Condition cond) {
	        super();
	        this.id = id;
	        this.priority = priority;
	        this.resourceTypeTable.addAll(wg.resourceTypeTable);
	    ...
	    protected boolean isFeasible(WorkItem wi) {
	        ...
	        if (conflict) { // The resource distribution algorithm is invoked
	        	debug("Overlapped resources with " + wi.getElement());
	        	...
	        	debug("Possible conflict. Recheck is needed " + wi.getElement());
	    ...
	    protected boolean hasSolution(int []pos, int []nec, WorkItem wi) {
	        for (int i = pos[0]; i < resourceTypeTable.size(); i++) {
	            ...
	            while (((res = actual.getResource(j)) != null) && (disp < nec[i])) {
	        		res.debug("MUTEX\trequesting\t" + wi.getElement() + "(solution)");    	
	        		res.waitSemaphore();
	        		res.debug("MUTEX\tadquired\t" + wi.getElement() + "(solution)");    	
	                ...
	                if (res.isBooked(wi) && (res.getCurrentWI() == null) && (res.getCurrentResourceType() == null))
	                    disp++;
	        		res.debug("MUTEX\treleasing\t" + wi.getElement() + " (solution)");    	
	        		res.signalSemaphore();
	        		res.debug("MUTEX\tfreed\t" + wi.getElement() + " (solution)");	                
	                j++;
	            }
	            if (disp < nec[i])
	                return false;
	    ...
	    public String toString() {
	    	return new String("(" + Activity.this + ")" + getDescription());
	    }
