...
public class AgeBasedTimeToEventCalculator implements ParameterCalculator {
	...
	public AgeBasedTimeToEventCalculator(final Object[][] ageRisks, DiseaseProgression destManifestation, ParameterCalculator rrCalculator) {
	...
	public double getValue(Patient pat) {
		...
		double time = Statistics.getAnnualBasedTimeToEvent((Double)ageRisks[interval][j+1], Math.log(rndValues.get(0)), rrCalculator.getValue(pat));
		
		// Checks if further intervals compute lower time to event
		for (; interval < ageRisks.length; interval++) {
			final double newTime = Statistics.getAnnualBasedTimeToEvent((Double) ageRisks[interval][j+1], Math.log(rndValues.get(rndValues.size() - (ageRisks.length - interval))), rrCalculator.getValue(pat));
