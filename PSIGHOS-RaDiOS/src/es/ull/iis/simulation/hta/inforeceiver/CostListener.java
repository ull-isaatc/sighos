		...
		if (info instanceof SimulationStartStopInfo) {
			...
			if (SimulationStartStopInfo.Type.END.equals(tInfo.getType())) {
				...
				for (int i = 0; i < lastTs.length; i++) {
					...
					if (!pat.isDead()) {
						final double initT = pat.getSimulation().getModel().simulationTimeToYears(TimeUnit.DAY.convert(lastTs[pat.getIdentifier()], simUnit)); 
			...
			case SCREEN:
				...
				break;
			case START_MANIF:
				update(pat, pInfo.getDiseaseProgression().getStartingCost(pat, endT, discountRate));
			case DEATH:
			case START:
				break;
			default:
