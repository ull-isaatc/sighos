...
public interface PopulationBuilder {
	 ...
	 * @return
	 */
	public static StdPopulation getPopulationInstance(SecondOrderParamsRepository secParams, Disease disease, String populationName) throws TranspilerException {
	...
	static class OSDiPopulation extends StdPopulation {
		...
		public OSDiPopulation(SecondOrderParamsRepository secParams, Disease disease, String populationName) throws TranspilerException {
		...
		public void registerSecondOrderParameters(SecondOrderParamsRepository secParams) {
			try {
				final String strPFemale = OSDiNames.DataProperty.HAS_FEMALE_PROPORTION.getValue(name(), "0.5");
				...
				List<String> epidemParams = OSDiNames.Class.EPIDEMIOLOGICAL_PARAMETER.getDescendantsOf(name());
		...
		private void registerUtilityParam(SecondOrderParamsRepository secParams) throws TranspilerException {
			List<String> utilities = OSDiNames.Class.UTILITY.getDescendantsOf(name());
			if (utilities.size() > 1)
			...
			for (String utilityName : utilities) {
				...
				final String strCalcMethod = OSDiNames.DataProperty.HAS_CALCULATION_METHOD.getValue(utilityName, "Unknown");
				try {
					final ProbabilityDistribution probDistribution = new ProbabilityDistribution(strValue);
					UtilityParamDescriptions.BASE_UTILITY.addParameter(secParams, name(), OSDiNames.DataProperty.HAS_DESCRIPTION.getValue(utilityName, "Utility for " + name() + " calculated using " + strCalcMethod),  
		...
		private void registerEpidemParam(SecondOrderParamsRepository secParams, String paramName, ProbabilityParamDescriptions secondOrderParam) throws TranspilerException {
			...
			if (!manifestations.isEmpty()) {
				...
				try {
					...
					for (String manifestationName: manifestations) {
						ProbabilityParamDescriptions.INITIAL_PROPORTION.addParameter(secParams, disease.getManifestation(manifestationName), OSDiNames.getSource(paramName),
				...
				if (found) {
					...
					try {
						final ProbabilityDistribution probabilityDistribution = new ProbabilityDistribution(strValue);
						secondOrderParam.addParameter(secParams, name(), name(), OSDiNames.getSource(paramName), 
		...
		protected List<ClinicalParameter> getPatientParameterList() {
			...
			List<String> indParams = OSDiNames.ObjectProperty.DEFINES_INDIVIDUAL_PARAMETER_VALUE.getValues(name());
			for (String paramName : indParams) {
				try {
					final String indParamParamName = OSDiNames.ObjectProperty.IS_VALUE_OF_INDIVIDUAL_PARAMETER.getValue(paramName);
					final String indParamName = OSDiNames.DataProperty.HAS_NAME.getValue(indParamParamName);
					final String strValue = OSDiNames.DataProperty.HAS_VALUE.getValue(paramName, "0.0");
					final ProbabilityDistribution probabilityDistribution = new ProbabilityDistribution(strValue);
