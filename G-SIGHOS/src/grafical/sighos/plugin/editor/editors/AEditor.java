...
public class AEditor extends EditorPart {

	public static final String ID = "grafical.sighos.plugin.editor.editors.AEditor";
	private A_property a_obj;
	private Text text2;
	private Text text3;
	private Text text4;	
	private Text text5;	
	...
	public void doSave(IProgressMonitor monitor) {
		a_obj.setName(text2.getText());
		a_obj.code_generator();
		inicial.refresh();
	...
	throws PartInitException {
		setSite(site);
		setInput(input);
		a_obj = ((AEditorInput) input).getA_property();
		setPartName(a_obj.getName());
	}
	@Override
	public boolean isDirty() {
		if (a_obj.getName().equals(text2.getText())) {
			return false;
		}
	...
	public void createPartControl(Composite parent) {
		...
		text9.setText(a_obj.getCode(1));		
		text9.setEditable(false);		
			
		
if ( a_obj.getCode().size() > 2) {
	
	Label  label51 = new Label(parent, SWT.CENTER);
	label51.setText("");
	//FIN RELLENO		
	text4 = new Text(parent, SWT.BORDER);	
	text4.setText(a_obj.getCode(2));		
	text4.setEditable(false);
}
if ( a_obj.getCode().size() > 3) {
