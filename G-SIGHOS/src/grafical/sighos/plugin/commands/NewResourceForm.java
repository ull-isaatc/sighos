...
public class NewResourceForm extends TitleAreaDialog {
   ...
   // private Spin
	...
	public NewResourceForm(Shell parentShell) {
		super(parentShell);
	}
	@Override
	public void create() {
	...
	protected Control createDialogArea(Composite parent) {
		...
		Integer ttNumber = new Integer( R_property.r_List.size() );
		...
		varNameText.setText("r_"+ttNumber.toString());
		varNameText.setEditable(false);

		
		
		Label label9 = new Label(parent, SWT.NONE);
		label9.setText("seleciona el numero ResourceType");
	...
	private boolean isValidInput() {
		...
		if (nameText.getText().length() == 0) {
			setErrorMessage("Nombre de 'Recurso' no valido");
			valid = false;
		}
		else {
			...
			int i =0;
			while ( i < R_property.r_List.size() ) {

				if ( R_property.r_List.get(i).getName().equalsIgnoreCase(nameText.getText())  ) {
					valid = false;
					setErrorMessage("Nombre de 'Recurso' Repetido");
				}
				i++;
			}
		}
