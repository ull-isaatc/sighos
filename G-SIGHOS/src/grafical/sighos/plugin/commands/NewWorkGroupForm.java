...
public class NewWorkGroupForm extends TitleAreaDialog {
   ...
   // private Spin
	...
	public NewWorkGroupForm(Shell parentShell) {
		super(parentShell);
	}
	@Override
	public void create() {
	...
	protected Control createDialogArea(Composite parent) {
		...
		Integer ttNumber = new Integer( W_property.w_List.size() );
		...
		varNameText.setText("w_"+ttNumber.toString());
		varNameText.setEditable(false);

		
		
		Label label9 = new Label(parent, SWT.NONE);
		label9.setText("seleciona el numero de Recursos");
	...
	private boolean isValidInput() {
		...
		if (nameText.getText().length() == 0) {
			setErrorMessage("Nombre de 'Workgroup' no valido");
			valid = false;
		}
		else {
			...
			int i =0;
			while ( i < W_property.w_List.size() ) {

				if ( W_property.w_List.get(i).getName().equalsIgnoreCase(nameText.getText())  ) {
					valid = false;
					setErrorMessage("Nombre de 'Workgroup' Repetido");
				}
				i++;
			}
		}
