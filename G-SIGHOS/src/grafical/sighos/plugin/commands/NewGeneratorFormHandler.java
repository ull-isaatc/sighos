...
public class NewGeneratorFormHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		if ( Application.initialized_proyect ) {
			NewGeneratorForm dialog = new NewGeneratorForm(HandlerUtil.getActiveWorkbenchWindow(
					event).getShell());
			dialog.create();
			...
			for (Object s : result) {
			   unitTime += s.toString();
				...
				G_property aux = new G_property( dialog.getName(),	dialog.getVarName(),
						dialog.getNumber(),code_generator );

				G_property.g_List.add(aux);
				inicial.g_branch.add(aux);
