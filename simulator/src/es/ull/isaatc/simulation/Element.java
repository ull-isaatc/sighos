...
public class Element extends BasicElement implements
	...
	protected void carryOutActivity(SingleFlow f, WorkGroup wg) {
		...
		print("Starts\t" + f.getActivity() + "\t" + f.getActivity().getDescription());
		...
		public void event() {
			simul.notifyListeners(new ElementInfo(Element.this,
					ElementInfo.Type.REQACT, ts, flow.getActivity()
							.getIdentifier()));
			print("Requests\t" + flow.getActivity() + "\t" + flow.getActivity().getDescription());
			// Beginning MUTEX access to activity manager
			flow.getActivity().getManager().waitSemaphore();
			Activity act = flow.getActivity();
    		print("MUTEX\trequesting\t" + act + " (req. act.)");    	
            waitSemaphore();
    		print("MUTEX\tadquired\t" + act + " (req. act.)");    	
			...
			if ((currentSF == null) || (!act.isPresential())) {
				...
				if (wg != null) { // There are enough resources to perform the
									// activity
            		print("MUTEX\treleasing\t" + act + " (req. act.)");    	
                	signalSemaphore();
            		print("MUTEX\tfreed\t" + act + " (req. act.)");    	
	...
	public class AvailableElementEvent extends BasicElement.DiscreteEvent {
		...
		public void event() {
			...
			Activity act = flow.getActivity();
    		print("MUTEX\trequesting\t" + act + " (av. el.)");    	
            waitSemaphore();
    		print("MUTEX\tadquired\t" + act + " (av. el.)");    	
			print("Calling availableElement()\t" + act + "\t" + act.getDescription());
			// If the element is not performing a presential activity yet
			if (currentSF == null) {
				WorkGroup wg = act.isFeasible(flow);
				if (wg != null) {
					print("Can carry out\t" + act + "\t" + wg);
					if (!act.queueRemove(flow))
						printError("Unexpected error. Element MUST BE in the activity queue\t" + act);
            		print("MUTEX\treleasing\t" + act + " (av. el.)");    	
                	signalSemaphore();
            		print("MUTEX\tfreed\t" + act + " (av. el.)");    	
					carryOutActivity(flow, wg);
				} else {
            		print("MUTEX\treleasing\t" + act + " (av. el.)");    	
                	signalSemaphore();
		...
		public void event() {
			simul.notifyListeners(new ElementInfo(Element.this,
					ElementInfo.Type.ENDACT, ts, flow.getActivity()
							.getIdentifier()));
			print("Finishes\t" + flow.getActivity() + "\t" + flow.getActivity().getDescription());
