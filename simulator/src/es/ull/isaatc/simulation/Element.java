 ...
 */
public class Element extends BasicElement implements
	RecoverableState<ElementState> {
    /** Element type */
    ...
    /**
     * If the element is currently performing an activity, returns the single flow that the 
     ...
     * @param currentSF The single flow that the element is going to use. A null value indicates
	...
	print(Output.MessageType.DEBUG, "Starts\t" + f.getActivity(),
		"Starts\t" + f.getActivity() + "\t"
			+ f.getActivity().getDescription());
    ...
    /**
     * Requests an activity. Checks if the activity is feasible and the element is not 
	...
	public void event() {
	    ...
	    print(Output.MessageType.DEBUG, "Requests\t" + flow.getActivity(),
		    "Requests\t" + flow.getActivity() + "\t"
			    + flow.getActivity().getDescription());
	...
	public void event() {
	    ...
	    Activity act = flow.getActivity();
	    waitSemaphore();
	    print(Output.MessageType.DEBUG, "Calling availableElement()\t"
		    + act, "Calling availableElement()\t" + act + "\t"
		    + act.getDescription());
	    // If the element is not performing a presential activity yet
	    if (currentSF == null) {
		WorkGroup wg = act.isFeasible(flow);
		if (wg != null) {
		    print(Output.MessageType.DEBUG, "Can carry out\t" + act,
			    "Can carry out\t" + act + "\t" + wg);
		    if (!act.queueRemove(flow)) { // saco el elemento de la cola
			print(
				Output.MessageType.ERROR,
				"Unexpected error. Element MUST BE in the activity queue",
				"Unexpected error. Element MUST BE in the activity queue\t"
					+ act);
		    }
		    signalSemaphore();
