...
public class T1DMMain {
	...
	private static final int N_RUNS = BasicConfigParams.BASIC_TEST_ONE_PATIENT ? 0 : 100;
	private static final SecondOrderParams secParams = BasicConfigParams.CHECK_CANADA ? new CanadaSecondOrderParams(true) : new BaseSecondOrderParams(true);
...
//		simul.addInfoReceiver(new T1DMPatientPrevalenceView(simul.getTimeUnit(), T1DMPatientPrevalenceView.buildAgesInterval(25, 90, 5, true)));
	...
	public static void main(String[] args) {
		...
		final Intervention[] interventions = secParams.getInterventions();
		T1DMTimeFreeOfComplicationsView timeFreeListener = new T1DMTimeFreeOfComplicationsView(BasicConfigParams.NPATIENTS, interventions.length, false);
		// First the deterministic simulation
		T1DMSimulation simul = new T1DMSimulation(0, true, interventions[0], new CommonParams(secParams), new ResourceUsageParams(secParams), new UtilityParams(secParams));
		...
		for (int i = 1; i <= N_RUNS; i++) {
			timeFreeListener = new T1DMTimeFreeOfComplicationsView(BasicConfigParams.NPATIENTS, interventions.length, false);
			simul = new T1DMSimulation(i, false, interventions[0], new CommonParams(secParams), new ResourceUsageParams(secParams), new UtilityParams(secParams));
			simul.addInfoReceiver(timeFreeListener);
			addListeners(simul);
			simul.run();
			simul = new T1DMSimulation(simul, interventions[1]);
