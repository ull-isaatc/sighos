...
public class DCCTSecondOrderParams extends SecondOrderParams {
	...
	private static final double YEARS_OF_EFFECT = 10.0;
	...
	public DCCTSecondOrderParams(boolean baseCase) {
		...
		final double[] paramsDNC_NEU = betaParametersFromNormal(0.0354, sdFrom95CI(CI_DNC_NEU));
		...
		addProbParam(new SecondOrderParam(STR_P_DNC_NEU, "Probability of healthy to clinically confirmed neuropathy, as processed in Sheffield Type 1 model", "DCCT 1995 https://doi.org/10.7326/0003-4819-122-8-199504150-00001", 0.0354, RandomVariateFactory.getInstance("BetaVariate", paramsDNC_NEU[0], paramsDNC_NEU[1])));
		addProbParam(new SecondOrderParam(STR_P_DNC_NPH, STR_P_DNC_NPH, "", 0.037));
		...
		addOtherParam(new SecondOrderParam(STR_RR_PREFIX + Complication.NEU.name(), "Beta for confirmed clinical neuropathy", "DCCT 1996 https://doi.org/10.2337/diab.45.10.1289, as adapted by Sheffield", 5.3));
		...
		addOtherParam(new SecondOrderParam(STR_AVG_BASELINE_AGE, "Average baseline age", "https://doi.org/10.1016/j.endinu.2018.03.008", 26));
		addOtherParam(new SecondOrderParam(STR_AVG_BASELINE_HBA1C, "Average baseline level of HBA1c", "https://doi.org/10.1016/j.endinu.2018.03.008", 8.8));
	}
	...
	public class DCCTIntervention2 extends T1DMMonitoringIntervention {
		...
		@Override
		public double getHBA1cLevel(T1DMPatient pat) {
			return 8.8 - (pat.isEffectActive() ? 1.9 : 0.0);
		}
