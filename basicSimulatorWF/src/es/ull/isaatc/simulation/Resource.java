...
public class Resource extends BasicElement implements Describable {
     ...
     * @param role Role that the resource plays during this cycle
     */
    public void addTimeTableEntry(SimulationCycle cycle, double dur, ResourceType role) {
    	addTimeTableEntry(cycle, new SimulationTime(simul.getUnit(), dur), role);
    }  
     ...
     * @param roleList Roles that the resource play during this cycle
     */
    public void addTimeTableEntry(SimulationCycle cycle, double dur, ArrayList<ResourceType> roleList) {
    	addTimeTableEntry(cycle, new SimulationTime(simul.getUnit(), dur), roleList);
    }  
    ...
    protected boolean releaseResource() {
		debug("MUTEX\trequesting\t" + currentWI.getElement() + "(rel. res.)");    	
		waitSemaphore();
		debug("MUTEX\tadquired\t" + currentWI.getElement() + "(rel. res.)");    	
		setTs(defLP.getTs());
...
//		debug("unbooked\t" + currentWI.getElement());
    ...
    public class RoleOnEvent extends BasicElement.DiscreteEvent {
        ...
        @Override
        public void event() {
        	double waitTime = role.beforeRollOn();
        	if (waitTime == 0.0) {
