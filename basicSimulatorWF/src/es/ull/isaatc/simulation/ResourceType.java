...
public class ResourceType extends TimeStampedSimulationObject implements Describable {
    ...
    protected int[] getAvailable(WorkItem wi) {
        ...
        for (int i = 0; i < availableResourceList.size(); i++) {
    		...
    		res.debug("MUTEX\trequesting\t" + wi.getElement() + "(get avail.)");    	
    		res.waitSemaphore();
    		res.debug("MUTEX\tadquired\t" + wi.getElement() + "(get avail.)");    	
    		...
    		res.debug("MUTEX\treleasing\t" + wi.getElement() + " (get avail.)");    	
    		res.signalSemaphore();
    		res.debug("MUTEX\tfreed\t" + wi.getElement() + " (get avail.)");    	
        }
        return total;
    }
    ...
    protected void resetAvailable(WorkItem wi) {
        for (int i = 0; i < availableResourceList.size(); i++) {
        	Resource res = availableResourceList.get(i);
    		res.debug("MUTEX\trequesting\t" + wi.getElement() + "(reset avail.)");    	
    		res.waitSemaphore();
    		res.debug("MUTEX\tadquired\t" + wi.getElement() + "(reset avail.)");    	
        	res.removeBook(wi);
    		res.debug("MUTEX\treleasing\t" + wi.getElement() + " (reset avail.)");    	
    		res.signalSemaphore();
    		res.debug("MUTEX\tfreed\t" + wi.getElement() + " (reset avail.)");    	
        }
    }
    ...
    protected int getNextAvailableResource(int ind, WorkItem wi) {
        for (; ind < availableResourceList.size(); ind++) {
    		...
    		res.debug("MUTEX\trequesting\t" + wi.getElement() + "(next avail.)");    	
    		res.waitSemaphore();
    		res.debug("MUTEX\tadquired\t" + wi.getElement() + "(next avail.)");
            ...
            if (res.isBooked(wi) && (res.isAvailable()) && (res.getCurrentResourceType() == null)) {
        		res.debug("MUTEX\treleasing\t" + wi.getElement() + " (next avail.)");    	
        		res.signalSemaphore();
        		res.debug("MUTEX\tfreed\t" + wi.getElement() + " (next avail.)");    	
            	return ind;
            }
    		res.debug("MUTEX\treleasing\t" + wi.getElement() + " (next avail.)");    	
    		res.signalSemaphore();
    ...
    protected double catchResources(int n, WorkItem wi) {
        ...
        debug("Decrease amount\t" + n + "\t" + wi.getElement());
        
        // When this point is reached, it is suppose that there are enough resources
        for (int i = 0; i < availableResourceList.size(); i++) {
            Resource res = availableResourceList.get(i);
    		res.debug("MUTEX\trequesting\t" + wi.getElement() + "(catch res.)");    	
    		res.waitSemaphore();
    		res.debug("MUTEX\tadquired\t" + wi.getElement() + "(catch res.)");    	
    		// MOD 11/11/08
                ...
                if (res.isAvailable()) {
                	...
                	if (res.getCurrentResourceType() == null) {
    	            	if (n > 0) {
    	            		...
    	            		n--;
    	                    debug("Resource taken\t" + res + "\t " + n + "\t" + wi.getElement());
    	            	}
                	}
                	...
                	// Theoretically, I have no need to check "n"
                        ...
                        if (n < 0) {
                        	error("More resources than expected\t"+ n + "\t" + wi.getElement());
                        }
                	}
                }
            	res.removeBook(wi);
            }
    		res.debug("MUTEX\treleasing\t" + wi.getElement() + " (catch res.)");    	
    		res.signalSemaphore();
    		res.debug("MUTEX\tfreed\t" + wi.getElement() + " (catch res.)");    	
        }
        // This check should be unneeded
        if (n > 0)
        	error("UNEXPECTED ERROR: Less resources than expected\t"+ n + "\t" + wi.getElement());
