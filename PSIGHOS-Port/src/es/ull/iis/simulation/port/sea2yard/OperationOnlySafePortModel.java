 ...
 */
public class OperationOnlySafePortModel extends Simulation {
	...
	private final SafeOperationUnloadActivity[] actUnloads;
	 ...
	 */
	public OperationOnlySafePortModel(StowagePlan plan, int id, int nGenericVehicles, double pError) {
	 ...
	 */
	public OperationOnlySafePortModel(StowagePlan plan, int id, int nGenericVehicles, double pError, long seed) {
	 ...
	 */
	public OperationOnlySafePortModel(StowagePlan plan, int id, int[] nVehicles, double pError) {
	 ...
	 * @param seed Random seed used to ensure that the random parameters of the simulation replicate those from a former simulation 
	 */
	public OperationOnlySafePortModel(StowagePlan plan, int id, int[] nVehicles, double pError, long seed) {
		...
		for (int bayId = 0; bayId < nBays; bayId++) {
			...
			if (!bay.isEmpty()) {
				...
				for (int i = bay.size() - 1; i > 0; i--) {
					...
					actUnloads[containerId1] = new SafeOperationUnloadActivity(this, containerId1, containerId2);
				}
