...
public class ResultFormatter {
	...
	public static void main(String[] args) {
		...
		String dir = "S:\\simulacion\\Benchmark\\Iris\\p2_res" + NELEM + "_cpu2\\";
		int nExp = 10;
		...
		for (int nProc : NPROCS) {
			// create a new sheet
			HSSFSheet s = wb.createSheet("" + nProc + " PROC");
			...
			for (int nTh : NTHREADS) {				
				...
				for (int i = 0; i < NACTS.length; i++) {
					try {
						inFiles[i] = new BufferedReader(new FileReader(dir + "resp" + nProc + "a" + NACTS[i] + "th" + nTh + "" + NELEM + ".txt"));
					} catch (FileNotFoundException e) {
						e.printStackTrace();
					}
				}
				double [][] results = new double[NACTS.length][nExp];
