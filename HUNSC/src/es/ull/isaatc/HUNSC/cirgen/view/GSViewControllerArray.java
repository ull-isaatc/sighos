 ...
 */
public class GSViewControllerArray {
	...
	public GSViewControllerArray(GSExcelInputWrapper input) {
		this.input = input;
	...
	private void writeWaitListenerResults() {
		...
		for (AdmissionType adm : AdmissionType.values())
			for (PatientType type : PatientType.values()) {
				...
				r.getCell((short)0).setCellStyle(diagStyle);
				numrow++;
				for (String day : GSElementTypeWaitView.REFDAYS) {
					r = s.createRow(numrow++);
				...
				double [][]refDays = new double[GSElementTypeWaitView.REFDAYS.length][waitList.length];
				for (short column = (short)ELEM_WAIT_HEADERS.length; column < waitList.length + ELEM_WAIT_HEADERS.length; column++) {
					...
					numrow++;
					for (int i = 0; i < GSElementTypeWaitView.REFDAYS.length; i++) {
	...
	public void notifyEnd(GSViewArray res) {
