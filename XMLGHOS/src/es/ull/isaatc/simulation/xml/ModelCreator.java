...
public class ModelCreator extends es.ull.isaatc.simulation.Simulation {
    ...
    Experiment xmlExperiment;

    /** List that matches a model ResourceType with an identifier */
    OrderedList<es.ull.isaatc.simulation.ResourceType> resType;
	...
	resType = new OrderedList<es.ull.isaatc.simulation.ResourceType>();
	for (ResourceType rtXML : xmlModel.getResourceType())
	    resType.add(new es.ull.isaatc.simulation.ResourceType(
		    rtXML.getId(), this, rtXML.getDescription()));
    }
	...
	for (Resource resXML : xmlModel.getResource()) {
		...
		if (resXML.getTimeTable().size() > 0) {
		    es.ull.isaatc.simulation.Resource resource = new es.ull.isaatc.simulation.Resource(
			    resXML.getId() + i, this, resXML.getDescription());
		    // Insert each time table entry as several roles entries
		    for (TimeTable timeTable : resXML.getTimeTable()) {
			ArrayList<es.ull.isaatc.simulation.ResourceType> roles = new ArrayList<es.ull.isaatc.simulation.ResourceType>();
			for (ComponentRef resTypeRef : timeTable.getRtRef())
			    roles.add(resType.get(Integer.valueOf(resTypeRef
				    .getId())));
			resource.addTimeTableEntry(createCycle(timeTable
				.getCycle()), (int) (timeTable.getDur()
	...
	for (Activity actXML : xmlModel.getActivity()) {
	    createWorkGroups(actXML, new es.ull.isaatc.simulation.Activity(
		    actXML.getId(), this, actXML.getDescription(), actXML
			    .getPriority(), actXML.isPresential()));
	}
    }
	...
	for (WorkGroup wgXML : actXML.getWorkGroup()) {
	    ...
	    for (Role role : wgXML.getRole())
		wgModel.add(resType.get(Integer
			.valueOf(role.getRtRef().getId())), role.getUnits());
	}
    }
	...
	for (ElementType etXML : xmlModel.getElementType())
	    new es.ull.isaatc.simulation.ElementType(etXML.getId(), this, etXML
		    .getDescription(), etXML.getPriority());
    }
	...
	createMetaFlows();
	for (Generator genXML : xmlExperiment.getGenerator())
	...
	for (RootFlow rfXML : xmlModel.getRootFlow())
     ...
     *                Metaflows parent
     */
    es.ull.isaatc.simulation.MetaFlow createMetaFlow(Flow XMLFlow,
	...
	if (XMLFlow instanceof SingleFlow) {
		...
		return new es.ull.isaatc.simulation.SingleMetaFlow(XMLFlow
			...
			getActivity(XMLsf.getActRef().getId()));
		...
		return new es.ull.isaatc.simulation.SingleMetaFlow(XMLFlow
			...
			rn, getActivity(XMLsf.getActRef().getId()));
	    else
		return new es.ull.isaatc.simulation.SingleMetaFlow(XMLFlow
			.getId(), rn, getActivity(XMLsf.getActRef().getId()));
	}
	...
	for (ComponentRef ref : XMLBranch.getElementType())
	    elementTypes.add(getElementType(ref.getId()));
	...
	if (probTree.getSubTree().size() == 0) {
	    probVector.add(new GenerationTrio(getElementType(probTree
		    .getElementType().getId()), flowList.get(probTree
		    .getMetaFlow().getId()), probTree.getProb()));
	    return probVector;
	}
	...
	public String toString() {
	    return new String("\tElementType : " + elementType
		    + "\tMetaFlow : " + metaFlow);
	}
    }
}
