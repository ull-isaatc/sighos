...
class StateSimulation extends Simulation {
	...
	StateSimulation(String desc, double startTs, double endTs,
			int complexityDegree) {
	...
	protected void createModel() {
		...
		switch (complexityDegree) {
			case 0:
				...
				for (int i = 0; i < NSIMPLE; i++)
					new Activity(i, this, "ACT" + i)
							.getNewWorkGroup(0, new Fixed(DURACT)).add(getResourceType(i), 1);
				for (int i = 0; i < NSIMPLE; i++)
					new ElementType(i, this, "ELEMT" + i);
				for (int i = 0; i < NSIMPLE; i++)
					new Resource(i, this, "RES" + i).addTimeTableEntry(cPeriod, DURRES,
							getResourceType(i));
				for (int i = 0; i < NSIMPLE; i++)
					new ElementGenerator(this, new Fixed(NELEM), cPeriod.iterator(
							startTs, endTs), getElementType(i), new SingleMetaFlow(i,
							new Fixed(1), getActivity(i)));
				break;
			case 1:
				for (int i = 0; i < NSIMPLE; i++)
					new ResourceType(i, this, "RT" + i);
				...
				for (int i = 0; i < NSIMPLE; i++)
					new ElementType(i, this, "ELEMT" + i);
				for (int i = 0; i < NSIMPLE; i++)
					new Resource(i, this, "RES" + i).addTimeTableEntry(cPeriod,
							DURRES + 20, getResourceType(i));
				Cycle c4 = new Cycle(0.0, new Fixed(PERIOD), 0);
				for (int i = 0; i < NSIMPLE; i++)
					new ElementGenerator(this, new Fixed(NELEM), c4.iterator(startTs,
							endTs), getElementType(i), new SingleMetaFlow(i, new Fixed(1),
							getActivity(i)));
				break;
			case 2:
				...
				new ElementGenerator(this, new Fixed(NELEM), cPeriod.iterator(startTs,
						endTs), et, new SingleMetaFlow(0, new Fixed(1), a));
			default:
				break;
		}
	}
...
class StateExperiment extends Experiment {
	...
	public Simulation getSimulation(int ind) {
		...
		StateSimulation sim = new StateSimulation("Simulation PART: " + ind,
				simTime * ind, simTime * (ind + 1), COMPLEXITY);
		return sim;
	}
	
	
	// public void start() {
	// Simulation prevSim = new StateSimulation("Simulation INITIAL", 0.0,
	// simTime, COMPLEXITY);
	// prevSim.start();
	// System.out.println("---------------------------------------------------------------");
