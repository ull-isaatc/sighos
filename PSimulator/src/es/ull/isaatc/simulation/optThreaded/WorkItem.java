...
import es.ull.isaatc.simulation.optThreaded.flow.SingleFlow;
    ...
    protected ArrayList<ActivityManager> releaseCaughtResources() {
        ...
        for (Resource res : caughtResources) {
			for (int i = 0; i < act.cancellationList.size(); i++) {
				es.ull.isaatc.simulation.optThreaded.Activity.CancelListEntry entry = act.cancellationList.get(i);
