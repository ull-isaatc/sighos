...
import es.ull.isaatc.simulation.groupedThreaded.flow.SingleFlow;
    ...
    protected ArrayList<ActivityManager> releaseCaughtResources() {
        ...
        for (Resource res : caughtResources) {
			for (int i = 0; i < act.cancellationList.size(); i++) {
				es.ull.isaatc.simulation.groupedThreaded.Activity.CancelListEntry entry = act.cancellationList.get(i);
