...
public class MarshallModelUtil {
	...
	public static Model unMarshallModel(Reader modelReader) {
		...
		try {
			...
			// u.setSchema(schemaFactory.newSchema(new
			// File("D:/isaatc/workspace/XMLGHOS/model.xsd")));
			u.setSchema(schemaFactory.newSchema(URLClassLoader
					.getSystemResource("model.xsd")));
			model = (Model) u.unmarshal(modelReader);
	...
	public static Experiment unMarshallExperiment(Reader experimentReader) {
		...
		try {
			...
			SchemaFactory schemaFactory = SchemaFactory
					.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
			// u.setSchema(schemaFactory.newSchema(new
			// File("D:/isaatc/workspace/XMLGHOS/experiment.xsd")));
			u.setSchema(schemaFactory.newSchema(URLClassLoader
					.getSystemResource("experiment.xsd")));
			experiment = (Experiment) u
					.unmarshal(experimentReader);
		} catch (JAXBException je) {
			System.out.println("ERROR : Error found in one of the XML files");
			je.printStackTrace();
			System.exit(-1);
