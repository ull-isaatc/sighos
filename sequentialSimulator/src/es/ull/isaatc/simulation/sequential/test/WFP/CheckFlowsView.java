...
public class CheckFlowsView extends WFPTestView {
	FlowNode flow;
	SimulationTime[] durations;
	ArrayList<TreeSet<EventToCheck>> futureFlow;
	private boolean ok = true;

	public CheckFlowsView(WFPTestSimulation simul, Flow f,
			SimulationTime[] durations) {
		this(simul, f, durations, true);
	}

	public CheckFlowsView(WFPTestSimulation simul, Flow f, SimulationTime[] durations, boolean detailed) {
		...
		futureFlow = new ArrayList<TreeSet<EventToCheck>>(
				WFPTestSimulation.DEFNELEMENTS);
	...
	public void infoEmited(SimulationInfo info) {
		...
		if (info instanceof ElementActionInfo) {
			...
			case STAACT:
				...
				printResult(futureFlow.get(eInfo.getElem().getIdentifier()).remove(ev), "unexpected event!"); 
				double nextTs = eInfo.getTs() + getSimul().simulationTime2Double(durations[eInfo.getActivity().getIdentifier()]);
		...
		if (f instanceof SingleFlow) {
			...
			if (sf.getSuccessor() == null)
				return new SingleFlowNode(sf.getIdentifier(), actId, getSimul()
						.simulationTime2Double(durations[actId]), null);
			return new SingleFlowNode(sf.getIdentifier(), actId, getSimul().simulationTime2Double(
